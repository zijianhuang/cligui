		...
		static void Main()
		{
			...
			switch (cliSettings.PluginAllocationMethod)
			{
				case PluginAllocationMethod.Mono:
					string assemblyName = cliSettings.AssemblyNameOfCommand;//Better to use the first in the list rather than AssemblyNameOfCommand which is in UserScope.
					if (String.IsNullOrEmpty(assemblyName))
						assemblyName = "RobocopyParametersCore";
					...
					else
					{
						...
						MessageBox.Show(String.Format("Cannot load {0} to initialize.", assemblyName));
					}
